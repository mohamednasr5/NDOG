...
const STAKING = {
  ...
  '365_days':   { apr: 0.50, days: 365,   min: 5000 },
  '1095_days':  { apr: 1.00, days: 1095,  min: 10000 },
};
...
function copyTextFn(text, successMsg) {
...
function copyText(text, successMsg) { copyTextFn(text, successMsg); }
...
function spinWheel() {
  ...
  // 25% chance of no prize
  const isNoPrize = Math.random() < 0.25;
  const prizeIdx  = isNoPrize ? -1 : Math.floor(Math.random() * WHEEL_PRIZES.length);
  const prize     = isNoPrize ? 0 : WHEEL_PRIZES[prizeIdx];
  const segDeg    = 360 / WHEEL_PRIZES.length;
  const actualIdx  = isNoPrize ? Math.floor(Math.random() * WHEEL_PRIZES.length) : prizeIdx;
  const targetDeg = 360 - (actualIdx * segDeg + segDeg / 2);
  ...
  setTimeout(() => {
    if (prize > 0) {
      ...
      if (spinResult) {
        spinResult.style.display = 'block';
        spinResult.textContent   = t('😢 حظ أوفر المرة القادمة!', '😢 Better luck next time!');
        setTimeout(() => { spinResult.style.display = 'none'; }, 4000);
      }
      showToast(t('😢 حظ أوفر المرة القادمة!', '😢 Better luck next time!'), 'info');
    }
    isSpinning = false;
...
function loadMissions() {
  db.ref('missions').once('value').then(snap => {
    const data = snap.val() || {};
    renderMissionsV2('dailyMissionsList',  data.daily  || {}, 'daily');
    renderMissionsV2('weeklyMissionsList', data.weekly || {}, 'weekly');
  });
}
...
function applyReferralCode(code) {
  ...
  return db.ref('referralCodes/' + code).once('value').then(snap => {
    ...
    return db.ref('users/' + currentUser.uid + '/referredBy').once('value').then(rs => {
      ...
      const now = Date.now();
      const newUserName = userData?.displayName || currentUser?.displayName || '';
      const upd = {};
      upd['users/' + currentUser.uid + '/referredBy'] = ref.uid;
      upd['referrals/' + ref.uid + '/' + currentUser.uid] = {
        referredAt: now,
      ...
      return db.ref().update(upd).then(() => {
        ...
        addBalance(50, 'referral_bonus_received');
        showToast(t('✅ كود مطبّق! +50 NDOG', '✅ Code applied! +50 NDOG'), 'success');
        setTimeout(() => { if (userData) { renderDashboard(); loadReferralCount(); loadReferralTree(); } }, 500);
      }).catch(console.error);
    });
  }).catch(console.error);
}
...
function shareReferral(platform) {
  ...
  if (urls[platform]) { window.open(urls[platform], '_blank'); trackReferralShare(); return; }
  if (platform === 'copy') {
    const code = userData.referralCode || '';
    const txt = webLink + '\n' + t('كود الدعوة: ', 'Referral Code: ') + code;
    copyTextFn(txt, t('📋 تم نسخ الرابط وكود الدعوة!', '📋 Link & code copied!'));
    trackReferralShare();
    return;
  }
}
...
function wireEvents() {
  ...
  // المحفظة
  q('walletSendBtn')?.addEventListener('click', () => openP2PTransfer());
  q('walletP2PBtn')?.addEventListener('click', () => openP2PTransfer());
  q('walletReceiveBtn')?.addEventListener('click', () => {
    if (!userData) return;
    const code = userData.referralCode || '';
    const link = window.location.origin + window.location.pathname + '?ref=' + code;
    const fullText = link + '\n' + t('كود الدعوة: ', 'Referral Code: ') + code;
    copyText(fullText, t('📋 تم نسخ الرابط وكود الدعوة!', '📋 Link & code copied!'));
  });
  ...
  // P2P Transfer modal
  q('p2pTransferModal')?.addEventListener('click', function (e) {
    if (e.target === this) closeP2PTransfer();
  });
  q('p2pTransferCloseBtn')?.addEventListener('click', closeP2PTransfer);
  q('p2pTransferSubmitBtn')?.addEventListener('click', executeP2PTransfer);

  // Biometric buttons
  q('enableBiometricBtn')?.addEventListener('click', enableFingerprint);
  q('disableBiometricBtn')?.addEventListener('click', disableFingerprint);
  q('fingerprintLoginBtn')?.addEventListener('click', loginWithFingerprint);
...
// 27. P2P TRANSFER
// ══════════════════════════════════════════════════════════
function openP2PTransfer() {
  const modal = q('p2pTransferModal');
  if (modal) modal.style.display = 'flex';
}

function closeP2PTransfer() {
  const modal = q('p2pTransferModal');
...
function executeP2PTransfer() {
  if (!currentUser || !userData) return;
  const codeInput = q('p2pReferralCode');
  const amountInput = q('p2pAmount');
  const code = (codeInput?.value || '').trim().toUpperCase();
  const amount = Number(amountInput?.value || 0);

  if (!code) { showToast(t('أدخل كود دعوة المستلم', 'Enter recipient referral code'), 'warning'); return; }
  if (amount < 5000) { showToast(t('الحد الأدنى للتحويل 5000 NDOG', 'Minimum transfer is 5000 NDOG'), 'warning'); return; }
  if (amount <= 0) { showToast(t('أدخل مبلغاً صحيحاً', 'Enter a valid amount'), 'warning'); return; }
  if (code === (userData.referralCode || '')) { showToast(t('لا يمكنك التحويل لنفسك!', 'Cannot transfer to yourself!'), 'error'); return; }

  showLoading();

  db.ref('referralCodes/' + code).once('value').then(snap => {
    if (!snap.exists()) { hideLoading(); showToast(t('كود الدعوة غير موجود', 'Referral code not found'), 'error'); return; }
    const recipientUid = snap.val().uid;
    if (recipientUid === currentUser.uid) { hideLoading(); showToast(t('لا يمكنك التحويل لنفسك!', 'Cannot transfer to yourself!'), 'error'); return; }

    const senderBal = Number(userData.balance) || 0;
    if (senderBal < amount) { hideLoading(); showToast(t('رصيد غير كافٍ', 'Insufficient balance'), 'error'); return; }
    ...
    db.ref('users/' + currentUser.uid).transaction(d => {
      ...
      if (!result.committed) { hideLoading(); showToast(t('رصيد غير كافٍ', 'Insufficient balance'), 'error'); return; }

      return db.ref('users/' + recipientUid).transaction(d => {
        if (!d) return d;
        d.balance = (Number(d.balance) || 0) + amount;
        return d;
      }).then(recResult => {
        if (!recResult.committed) {
          return db.ref('users/' + currentUser.uid + '/balance').transaction(d => { return (Number(d) || 0) + amount; }).then(() => {
            hideLoading(); showToast(t('فشل التحويل - تم استرداد المبلغ', 'Transfer failed - refunded'), 'error');
          });
        }

        const ts = firebase.database.ServerValue.TIMESTAMP;
        const senderTxId = db.ref().child('transactions').push().key;
        const recTxId = db.ref().child('transactions').push().key;
        const updates = {};
        updates['transactions/' + senderTxId] = { uid: currentUser.uid, amount: -amount, ts, type: 'p2p_send', platform: isTelegramEnv ? 'telegram' : 'web', meta: { toUid: recipientUid, toCode: code } };
        updates['transactions/' + recTxId] = { uid: recipientUid, amount: amount, ts, type: 'p2p_receive', platform: 'web', meta: { fromUid: currentUser.uid, fromCode: userData.referralCode } };
        ...
        return db.ref().update(updates).then(() => {
          ...
          closeP2PTransfer();
          showToast(t('✅ تم تحويل ' + amount.toLocaleString() + ' NDOG بنجاح!', '✅ Transferred ' + amount.toLocaleString() + ' NDOG!'), 'success');
        });
      });
    });
  }).catch(err => { hideLoading(); console.error('[NDOG] P2P error:', err); showToast(t('فشل التحويل', 'Transfer failed'), 'error'); });
}

// ══════════════════════════════════════════════════════════
// 28. FINGERPRINT / BIOMETRIC LOGIN
// ══════════════════════════════════════════════════════════
async function enableFingerprint() {
  if (!window.PublicKeyCredential) { showToast(t('المتصفح لا يدعم البصمة', 'Browser does not support biometrics'), 'error'); return; }
  try {
    const userId = new Uint8Array(currentUser.uid.slice(0, 32).padEnd(32, '0').split('').map(c => c.charCodeAt(0)));
    const createOptions = {
      publicKey: {
        challenge: new Uint8Array(32),
        rp: { name: 'NDOG Coin', id: window.location.hostname },
        user: { id: userId, name: userData?.email || currentUser.email || 'ndog-user', displayName: userData?.displayName || currentUser.displayName || 'NDOG User' },
    ...
    if (credential) {
      ...
      await db.ref('users/' + currentUser.uid + '/biometricCredId').set(credId);
      localStorage.setItem('ndog_biometric_uid', currentUser.uid);
      showToast(t('✅ تم تفعيل تسجيل الدخول بالبصمة!', '✅ Fingerprint login enabled!'), 'success');
      updateBiometricUI();
    }
  } catch (err) {
    console.error('[NDOG] Biometric enroll error:', err);
    if (err.name === 'NotAllowedError') { showToast(t('تم إلغاء عملية البصمة', 'Biometric cancelled'), 'warning'); }
    else { showToast(t('فشل تفعيل البصمة', 'Failed to enable fingerprint'), 'error'); }
  }
}

async function disableFingerprint() {
  try {
    await db.ref('users/' + currentUser.uid + '/biometricCredId').remove();
    localStorage.removeItem('ndog_biometric_uid');
    showToast(t('تم إلغاء البصمة', 'Fingerprint disabled'), 'info');
    updateBiometricUI();
  } catch (err) { showToast(t('فشل', 'Failed'), 'error'); }
}

async function loginWithFingerprint() {
  const savedUid = localStorage.getItem('ndog_biometric_uid');
  if (!savedUid) { showToast(t('البصمة غير مفعلة', 'Fingerprint not enabled'), 'warning'); return; }
  try {
    const getCredOptions = { publicKey: { challenge: new Uint8Array(32), allowCredentials: [], timeout: 60000, userVerification: 'required' } };
    const assertion = await navigator.credentials.get(getCredOptions);
    if (assertion) { showToast(t('✅ تم تسجيل الدخول بالبصمة!', '✅ Logged in with fingerprint!'), 'success'); }
  } catch (err) {
    console.error('[NDOG] Fingerprint login error:', err);
    if (err.name !== 'NotAllowedError') { showToast(t('فشل تسجيل الدخول بالبصمة', 'Fingerprint login failed'), 'error'); }
  }
}

function updateBiometricUI() {
  const enableBtn = q('enableBiometricBtn');
  const disableBtn = q('disableBiometricBtn');
  const statusEl = q('biometricStatus');
  if (!userData?.biometricCredId) {
    if (enableBtn) enableBtn.style.display = 'flex';
    if (disableBtn) disableBtn.style.display = 'none';
    if (statusEl) statusEl.textContent = t('غير مفعّل', 'Disabled');
  } else {
    if (enableBtn) enableBtn.style.display = 'none';
    if (disableBtn) disableBtn.style.display = 'flex';
    if (statusEl) statusEl.textContent = t('مفعّل ✅', 'Enabled ✅');
  }
}

// ══════════════════════════════════════════════════════════
// 29. MISSION VERIFICATION (Rewards only on actual completion)
// ══════════════════════════════════════════════════════════
function renderMissionsV2(containerId, missions, category) {
  const el = q(containerId);
  if (!el) return;
  el.innerHTML = '';
  const keys = Object.keys(missions);
  if (!keys.length) { el.innerHTML = '<div class="mission-empty">' + t('لا توجد مهام حالياً', 'No missions available') + '</div>'; return; }

  keys.sort((a, b) => (missions[a].order || 0) - (missions[b].order || 0));
  ...
  keys.forEach(id => {
    ...
    const title = currentLang === 'ar' ? (m.title_ar || m.title) : (m.title_en || m.title);
    const desc  = currentLang === 'ar' ? (m.desc_ar  || m.desc || '') : (m.desc_en  || m.desc || '');
    const missionType = m.type || m.missionType || 'generic';
    const reward = m.reward || 0;

    const div = document.createElement('div');
    div.className = 'mission-item';
    div.innerHTML = '<div class="mission-info"><div class="mission-title">' + title + '</div>' +
      (desc ? '<div class="mission-desc">' + desc + '</div>' : '') +
      '<div class="mission-reward">+' + reward + ' NDOG</div></div>' +
      '<button class="btn-mini mission-claim" data-id="' + id + '" data-reward="' + reward + '" data-type="' + missionType + '" data-category="' + category + '" type="button">' +
      t('مطالبة', 'Claim') + '</button>';
    el.appendChild(div);
  });

  el.querySelectorAll('.mission-claim').forEach(btn => {
    btn.addEventListener('click', function () {
      verifyAndCompleteMission(this.dataset.id, +this.dataset.reward, this.dataset.type, this.dataset.category);
    });
  });
}

function verifyAndCompleteMission(missionId, reward, missionType, category) {
  if (!currentUser) return;
  ...
  progressRef.once('value').then(snap => {
    ...
    switch(missionType) {
      case 'daily_login':
        var lastClaim = userData?.lastClaimAt || 0;
        var today = new Date(); today.setHours(0,0,0,0);
        verified = lastClaim > today.getTime();
        if (!verified) showToast(t('يجب تسجيل الدخول اليومي أولاً', 'Complete daily login first'), 'warning');
        verifyPromise = Promise.resolve();
        break;
      case 'share_referral':
        verifyPromise = db.ref('users/' + currentUser.uid + '/lastShareAt').once('value').then(s => {
          var lastShare = s.val() || 0;
          var td = new Date(); td.setHours(0,0,0,0);
          if (lastShare > td.getTime()) { verified = true; }
          else { showToast(t('يجب مشاركة رابط الإحالة أولاً', 'Share your referral link first'), 'warning'); }
        });
        break;
      case 'spin_wheel':
        verifyPromise = db.ref('users/' + currentUser.uid + '/lastWheelSpinAt').once('value').then(s => {
          var lastSpin = s.val() || 0;
          var td = new Date(); td.setHours(0,0,0,0);
          if (lastSpin > td.getTime()) { verified = true; }
          else { showToast(t('يجب تدوير عجلة الحظ أولاً', 'Spin the wheel first'), 'warning'); }
        });
        break;
      case 'read_whitepaper':
        verifyPromise = db.ref('users/' + currentUser.uid + '/whitepaperReadAt').once('value').then(s => {
          var readAt = s.val() || 0;
          if (readAt > 0) { verified = true; }
          else { showToast(t('يجب قراءة الورقة البيضاء أولاً', 'Read the whitepaper first'), 'warning'); }
        });
        break;
      case 'streak_5':
        if ((userData?.streak || 0) >= 5) { verified = true; verifyPromise = Promise.resolve(); }
        else { showToast(t('يجب تحقيق 5 أيام متتالية (حالياً: ' + (userData?.streak || 0) + ')', 'Need 5-day streak (current: ' + (userData?.streak || 0) + ')'), 'warning'); verifyPromise = Promise.resolve(); }
        break;
      case 'streak_7':
        if ((userData?.streak || 0) >= 7) { verified = true; verifyPromise = Promise.resolve(); }
        else { showToast(t('يجب تحقيق 7 أيام متتالية (حالياً: ' + (userData?.streak || 0) + ')', 'Need 7-day streak (current: ' + (userData?.streak || 0) + ')'), 'warning'); verifyPromise = Promise.resolve(); }
        break;
      case 'invite_3':
        verifyPromise = db.ref('referrals/' + currentUser.uid).once('value').then(s => {
          var count = s.numChildren();
          if (count >= 3) { verified = true; }
          else { showToast(t('يجب دعوة 3 أصدقاء (حالياً: ' + count + ')', 'Invite 3 friends (current: ' + count + ')'), 'warning'); }
        });
        break;
      default:
        verified = true; verifyPromise = Promise.resolve();
    }
    ...
    if (verifyPromise) {
      verifyPromise.then(() => {
        if (!verified) return;
        showLoading();
        progressRef.set({ completedAt: firebase.database.ServerValue.TIMESTAMP, reward, type: missionType })
          .then(() => addBalance(reward, 'mission_' + missionId))
          .then(() => { hideLoading(); showToast(t('✅ مهمة مكتملة! +' + reward + ' NDOG', '✅ Mission done! +' + reward + ' NDOG'), 'success'); loadMissions(); })
          .catch(() => { hideLoading(); showToast(t('فشل', 'Failed'), 'error'); });
      });
...
function trackWhitepaperRead() {
  if (!currentUser) return;
  db.ref('users/' + currentUser.uid + '/whitepaperReadAt').set(firebase.database.ServerValue.TIMESTAMP);
}

function trackReferralShare() {
  if (!currentUser) return;
  db.ref('users/' + currentUser.uid + '/lastShareAt').set(firebase.database.ServerValue.TIMESTAMP);
}
...
function loadReferralTree() {
  if (!currentUser) return;
  var l1El = q('refTreeL1'), l2El = q('refTreeL2'), l3El = q('refTreeL3');
  if (l1El) l1El.innerHTML = '';
  if (l2El) l2El.innerHTML = '';
  if (l3El) l3El.innerHTML = '';
  ...
  db.ref('referrals/' + currentUser.uid).once('value').then(snap => {
    ...
    snap.forEach(child => {
      ...
      if (l1El) {
        ...
        m.className = 'ref-tree-member';
        m.innerHTML = '<span class="ref-member-avatar">' + (ref.displayName || '?').charAt(0).toUpperCase() + '</span><span class="ref-member-name">' + (ref.displayName || t('مستخدم', 'User')) + '</span>';
        l1El.appendChild(m);
      }
      // Level 2
      db.ref('referrals/' + child.key).once('value').then(l2Snap => {
        l2Snap.forEach(l2Child => {
          var l2Ref = l2Child.val();
          if (l2El) {
            var m2 = document.createElement('div');
            m2.className = 'ref-tree-member';
            m2.innerHTML = '<span class="ref-member-avatar">' + (l2Ref.displayName || '?').charAt(0).toUpperCase() + '</span><span class="ref-member-name">' + (l2Ref.displayName || t('مستخدم', 'User')) + '</span>';
            l2El.appendChild(m2);
          }
          // Level 3
          db.ref('referrals/' + l2Child.key).once('value').then(l3Snap => {
            l3Snap.forEach(l3Child => {
              var l3Ref = l3Child.val();
              if (l3El) {
                var m3 = document.createElement('div');
                m3.className = 'ref-tree-member';
                m3.innerHTML = '<span class="ref-member-avatar">' + (l3Ref.displayName || '?').charAt(0).toUpperCase() + '</span><span class="ref-member-name">' + (l3Ref.displayName || t('مستخدم', 'User')) + '</span>';
                l3El.appendChild(m3);
              }
            });
          });
